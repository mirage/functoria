...
val package :
  ...
  string ->
  package
(** [package ~build ~sublibs ~ocamlfind ~min ~max ~pin opam] is a [package].
    [Build] indicates a build-time dependency only, defaults to [false]. The
    ocamlfind name is by default the same as [opam], you can specify [~sublibs]
    to add additional sublibraries (e.g. [~sublibs:\["mirage"\] "foo"] will
    result in the findlib names [ \["foo"; "foo.mirage"\] ]. In case the findlib
    name is disjoint (or empty), use [~ocamlfind]. Specifying both [~ocamlfind]
    and [~sublibs] leads to an invalid argument. Version constraints are given
    as [min] (inclusive) and [max] (exclusive). If [pin] is provided, a
...
(** {1:app Application Builder}
    ...
    {!foreign} construct. Module implementations of type {!job} can then be
    {{!Functoria_app.Make.register} registered} into an application builder. The
    builder is in charge if parsing the command-line arguments and of generating
    code for the final application. See {!Functoria_app} for details. *)
...
(** [foreign name typ] is the module [name], having the module type [typ].
    ...
    - If [packages] is set, then the given packages are installed before
      compiling the current application.
...
module Info : sig
  ...
  val create :
    ...
    build_dir:Fpath.t ->
    t
  (** [create context n r] contains information about the application being
      built. *)
...
(** Signature for configurable module implementations. A [configurable] is a
    module implementation which contains a runtime state which can be set either
    at configuration time (by the application builder) or at runtime, using
    command-line arguments. *)
class type ['ty] configurable =
  object
    method ty : 'ty typ
    (** [ty] is the module type of the configurable. *)

    method name : string
    (** [name] is the unique variable name holding the runtime state of the
        configurable. *)
    ...
    (** [packages] is the list of OPAM packages which needs to be installed
        before compiling the configurable. *)

    method connect : Info.t -> string -> string list -> string
    (** [connect info mod args] is the code to execute in order to initialize
        the state associated with the module [mod] (usually calling
        [mod.connect]) with the arguments [args], in the context of the project
        information [info]. *)

    method configure : Info.t -> (unit, Rresult.R.msg) result
    (** [configure info] is the code to execute in order to configure the
        device. During the configuration phase, the specficied {!packages} might
        not yet be there. The code might involve generating more OCaml code,
        running shell scripts, etc. *)

    method build : Info.t -> (unit, Rresult.R.msg) result
    (** [build info] is the code to execute in order to build the device. During
        the build phase, you can rely that all {!packages} are installed (via
        opam). The code might involve generating more OCaml code (crunching
        directories), running shell scripts, etc. *)

    method clean : Info.t -> (unit, Rresult.R.msg) result
    (** [clean info] is the code to clean-up what has been generated by {!build}
        and {!configure}. *)
    ...
    (** [deps] is the list of {{!abstract_impl} abstract implementations} that
        must be initialized before calling {!connect}. *)
  end

val impl : 'a configurable -> 'a impl
(** [impl c] is the implementation of the configurable [c]. *)

(** [base_configurable] pre-defining many methods from the {!configurable}
...
class ['a] foreign :
  ...
  -> 'a typ
  -> ['a] configurable
(** This class can be inherited to define a {!configurable} with an API similar
    to {!foreign}.
    ...
    {[
      let main =
        impl
        @@ object
             inherit [_] foreign "Unikernel.Main" (console @-> job)
