...
module Arg = struct
  ...
  let compare_kind : type a b. a kind -> b kind -> int = fun a b ->
...
and equal_setters : type a b. a setter list -> b setter list -> bool =
  fun x y ->
    ...
    && List.for_all2 (fun (Setter (x, _)) (Setter (y, _)) ->
         equal (Any x) (Any y)
       ) x y
...
let rec hash (Any x) =
  Hashtbl.hash (
    Hashtbl.hash x.name,
    Arg.hash x.arg,
    List.map hash_setter x.setters
  )
...
and compare_setters: type a b. a setter list -> b setter list -> int =
  ...
  | (Setter (x, _))::tx, (Setter (y, _))::ty ->
     match compare (Any x) (Any y) with
     | 0 -> compare_setters tx ty
     | i -> i
