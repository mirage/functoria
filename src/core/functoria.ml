...
module Package = struct
        ...
        Some
          {
            ...
            build;
            ocamlfind;
            min = String.Set.empty;
            max = String.Set.empty;
            pin;
          }
    | None -> Some { opam; build; ocamlfind; min; max; pin }

  let package ?(build = false) ?sublibs ?ocamlfind ?min ?max ?pin opam =
    let ocamlfind =
      match (sublibs, ocamlfind) with
      | None, None -> [ opam ]
      ...
      | Some _, Some _ ->
          invalid_arg
            ("dependent package "
            ^ opam
            ^ " may either specify ~sublibs or ~ocamlfind")
    in
  ...
  let libraries ps =
    ...
    String.Set.elements
      (List.fold_left String.Set.union String.Set.empty (List.map ocamlfind ps))
...
module Info = struct
  ...
  let pins t =
    List.fold_left
      (fun acc p ->
        match p.pin with None -> acc | Some u -> (p.opam, u) :: acc)
      [] (packages t)
    ...
    let packages =
      List.fold_left
        (fun m p ->
          ...
          | Some p' -> (
              match Package.merge p.opam p p' with
  ...
  let pp_packages ?(surround = "") ?sep ppf t =
    Fmt.pf ppf "%a"
      (Fmt.iter ?sep List.iter (Package.pp_package surround))
      (packages t)
...
class ['ty] foreign ?(packages = []) ?(keys = []) ?(deps = []) module_name ty :
  ['ty] configurable =
...
let rec equal : type t1 t2. t1 impl -> t2 impl -> bool =
 fun x y ->
  ...
  | Impl _, (If _ | App _) | App _, (If _ | Impl _) | If _, (App _ | Impl _) ->
      false
...
let rec hash : type t. t impl -> int = function
  | Impl c ->
      Hashtbl.hash (c#name, List.map Key.hash c#keys, List.map hash_any c#deps)
