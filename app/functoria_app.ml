...
let keys (argv : argv impl) =
  ...
  @@ object
       ...
       method! connect info modname =
         function
         | [ argv ] ->
             Fmt.strf
               "return (Functoria_runtime.with_argv (List.map fst \
                %s.runtime_keys) %S %s)"
...
let pp_packages fmt l =
  Fmt.pf fmt "[@ %a]"
    Fmt.(
      iter ~sep:(unit ";@ ") List.iter @@ fun fmt x ->
      pf fmt "%S, \"%%{%s:version}%%\"" x x)
    l

let pp_dump_pkgs module_name fmt (name, pkg, libs) =
  Fmt.pf fmt
    "%s.{@ name = %S;@ @[<v 2>packages = %a@]@ ;@ @[<v 2>libraries = %a@]@ }"
    module_name name pp_packages (String.Set.elements pkg) pp_libraries
    (String.Set.elements libs)

let app_info ?(type_modname = "Functoria_runtime.Info")
    ?(gen_modname = "Info_gen") () =
  impl
  @@ object
       inherit base_configurable
       method ty = info
       ...
       method! build i =
         ...
         let rec opam_deps args collected =
           Log.debug (fun m ->
               m "opam_deps %d args %d collected\nargs: %a\ncollected: %a"
                 (String.Set.cardinal args)
                 (String.Set.cardinal collected)
                 (String.Set.pp ~sep:(Fmt.unit ",") Fmt.string)
                 args
                 (String.Set.pp ~sep:(Fmt.unit ",") Fmt.string)
                 collected);
           if String.Set.is_empty args then Ok collected
             ...
             let cmd =
               Bos.Cmd.(
                 ...
                 % pkgs)
             in
             Bos.OS.Cmd.run_out cmd |> Bos.OS.Cmd.out_lines
             >>= fun (rdeps, _) ->
         ...
         in
         opam_deps (String.Set.of_list (Info.package_names i)) String.Set.empty
         >>= fun opam ->
         let ocl = String.Set.of_list (Info.libraries i) in
         Bos.OS.File.writef
           Fpath.(file + "in")
           "@[<v 2>let info = %a@]"
           (pp_dump_pkgs type_modname)
...
module Engine = struct
  ...
  let module_name c id args =
    ...
    if args = [] then base
      ...
      let prefix =
        match String.cut ~sep:"." base with Some (l, _) -> l | None -> base
      in
  ...
  let find_device info g impl =
    ...
    let rec name : type a. a impl -> string =
     fun impl ->
  ...
  let build info (_init, job) =
    let f v =
      match Graph.explode job v with
      | `App _ | `If _ -> R.ok ()
    ...
    let f v =
      ...
      | `Impl (c, `Args args, `Deps _) ->
          ...
          if args = [] then ()
            ...
            Codegen.append_main "@[<2>module %s =@ %a@]" modname
              (module_expression tbl) (c, args);
            Codegen.newline_main ())
    in
    let f v res = res >>= fun () -> f v in
    Graph.fold f job @@ R.ok () >>| fun () -> tbl
    ...
    let f v =
      ...
      | `Impl (c, `Args args, `Deps deps) ->
          ...
          Codegen.append_main "%a" emit_connect
            (ident, names, c#connect info modname)
    in
    Graph.fold (fun v () -> f v) job ();
    let main_name = Graph.Tbl.find tbl @@ Graph.find_root job in
    let init_names =
      List.map
        (fun name -> Graph.Tbl.find tbl @@ find_device info job name)
        init
    in
  ...
  let configure_and_connect info g =
    configure info g >>| fun modtbl -> connect modtbl info g

  let clean i g =
    let f v =
      match Graph.explode g v with `Impl (c, _, _) -> c#clean i | _ -> R.ok ()
    in
    let f v res = res >>= fun () -> f v in
    Graph.fold f g @@ R.ok ()
end
...
module Config = struct
  ...
  let extract_keys impl = Engine.keys @@ Graph.create impl
  let keys t = t.keys
...
module Make (P : S) = struct
  ...
  let init_global_state argv =
    ...
    ignore
      (Cmdliner.Term.eval_peek_opts ~argv
  ...
  let get_build_dir () =
    let dir =
      match !build_dir with None -> get_relative_source_dir () | Some p -> p
    in
    let dir =
  ...
  let generate_dune_config ~project_root ~source_dir () =
    ...
    let pkgs =
      ...
      | pkgs ->
          let pkgs =
            List.fold_left
              (fun acc pkg -> String.Set.union pkg.ocamlfind acc)
...
|}
        ...
        copy_rules config_file config_file pkgs
    in
  ...
  let generate_configuration_files ~project_root ~source_dir ~build_dir
      ~config_file =
  ...
  let build_and_execute ?help_ppf ?err_ppf argv =
    ...
    generate_configuration_files ~project_root ~source_dir ~build_dir
      ~config_file
    >>= fun () ->
  ...
  let configure_main ~argv i jobs =
    ...
    Engine.configure_and_connect i jobs >>| fun () -> Codegen.newline_main ()
  ...
  let run_configure_with_argv argv config =
    ...
    handle_parse_args_result argv
      (Cmd.parse_args ~name:P.name ~version:P.version ~configure ~describe
