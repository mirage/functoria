...
let test_keys () =
  let k1 = Key.(abstract @@ create "foo" Arg.(opt int 1 (info [ "foo" ]))) in
  let k2 = Key.(abstract @@ create "foo" Arg.(opt int 2 (info [ "foo" ]))) in
  let k3 = Key.(abstract @@ create "foo" Arg.(opt int 1 (info [ "foo" ]))) in
...
let test_configure () =
  ...
  in
  let result =
    Cmd.parse_args ~name:"name" ~version:"0.2" ~configure:extra_term
...
let test_build () =
  ...
  in
  Alcotest.(check result_t) "build" (`Ok (Cmd.Build (true, true))) result
...
let test_clean () =
  ...
  let result =
    Cmd.parse_args ~name:"name" ~version:"0.2" ~configure:extra_term
      ...
      [| "name"; "clean" |]
  in
  Alcotest.(check result_t) "clean" (`Ok (Cmd.Clean (false, false))) result
...
let test_read_full_eval () =
  ...
  check "test" None (Cmd.read_full_eval [| "test" |]);
  check "test --eval" (Some true) (Cmd.read_full_eval [| "test"; "--eval" |]);
