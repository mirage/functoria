...
let test_configure () =
  ...
  Alcotest.(check files)
    "nothing should be created in the source dir" [ "app.ml"; "config.ml" ]
    (list_files root);
  Alcotest.(check files)
    "other files should be created in custom_build_"
    [
      "main.ml";
      "key_gen.ml";
      ".mirage.config";
...
let test_describe () =
  Test_app.run_with_argv
    [| ""; "describe"; "-vv"; "--file"; "tests/app/config.ml" |]
...
let test_build () =
  ...
  Alcotest.(check bool)
    "main.exe should be built in custom_build_" true
  ...
  Alcotest.(check bool)
    "toto.exe should be built in custom_build_" true
...
let test_keys () =
  ...
  Alcotest.(check string) "vote contains dog" "dog" (read_file "tests/app/vote");
  clean_app ()
...
let test_clean () =
  ...
  Alcotest.(check files)
    "clean should remove all the files" []
    (list_files (Fpath.v "custom_build_"))
